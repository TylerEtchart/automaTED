...
def _eunn_param(hidden_size, name, capacity=2, fft=False, comp=True):
    """
    ...
    if fft:
        ...
        params_theta = vs.get_variable("theta_0_" + name, [varsize], initializer=theta_phi_initializer)
        cos_theta = math_ops.cos(params_theta)
        sin_theta = math_ops.sin(params_theta)

        if comp:
            params_phi = vs.get_variable("phi_0_" + name, [varsize], initializer=theta_phi_initializer)
        ...
        params_theta_0 = vs.get_variable("theta_0_" + name, [capacity_a, hidden_size_a], initializer=theta_phi_initializer)
        ...
        params_theta_1 = vs.get_variable("theta_1_" + name, [capacity_b, hidden_size_b], initializer=theta_phi_initializer)
        ...
        if comp:
            params_phi_0 = vs.get_variable("phi_0_" + name, [capacity_a, hidden_size_a], initializer=theta_phi_initializer)
            ...
            params_phi_1 = vs.get_variable("phi_1_" + name, [capacity_b, hidden_size_b], initializer=theta_phi_initializer)
...
class GORUCell(RNNCell):
    ...
    def __init__(self, hidden_size, name, capacity=2, fft=False, activation=modrelu):
        ...
        self.diag_vec, self.off_vec, self.diag, self._capacity = _eunn_param(hidden_size, name, capacity, fft, False)
