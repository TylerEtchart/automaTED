...
targ_ph = tf.placeholder( tf.float32, [ batch_size, sequence_length, 100 ], name="targets")
...
def sample( num=200, prime='ab' ):
    ...
    for n in range(num):
        ...
        word_vec = np.random.normal( loc=mean_vec, scale=np.random.normal(loc=.5, scale=.1, size=100))
